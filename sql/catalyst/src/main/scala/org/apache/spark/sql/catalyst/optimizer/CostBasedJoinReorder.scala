...
import org.apache.spark.sql.internal.SQLConf.{LAST_LEVEL_SIZE, SPARK_COST_CARD, SPARK_COST_SIZE}
...
object CostBasedJoinReorder extends Rule[LogicalPlan] with PredicateHelper {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    // logWarning(s"Enter CostBasedJoinReorder!!!")
  ...
  private def reorder(plan: LogicalPlan, output: Seq[Attribute]): LogicalPlan = {
    ...
    // if (conf.itemSizeEnabled) {
    //   conf.setConf(ITEM_SIZE_CURRENT, items.size)
    // }
//    logWarning(s"reorderItemSize is ${items.size} items are ${items}")
    val result =
...
object JoinReorderDP extends PredicateHelper with Logging {
  ...
  def search(
    ...
    logWarning(s"planGroupEnabled is ${conf.planGroupEnabled}" +
      s" maxLastLevelSize is ${conf.maxLastLevel}")
    if (!conf.planGroupEnabled) {
      ...
      // logWarning(s"sparkFinalPlanCost is ${foundPlans.last.head._2.planCost}")
      if (conf.sparkCostRecordEnabled) {
        conf.setConf(SPARK_COST_CARD, foundPlans.last.head._2.planCost.card.toLong)
        conf.setConf(SPARK_COST_SIZE, foundPlans.last.head._2.planCost.size.toLong)
      }
//      logWarning(s"myItemLength is ${items.length}")
      ...
      val startTime = System.nanoTime()
      val itemIndex = items.zipWithIndex
      val foundPlans = mutable.Buffer[JoinPlanMapSetQueue]({
        val joinPlanMapSetQueue = new JoinPlanMapSetQueue
        itemIndex.foreach {
          case (item, id) =>
            joinPlanMapSetQueue.put(Set(id), mutable.PriorityQueue(JoinPlan(Set(id),
              item, ExpressionSet(), Cost(0, 0)))(PlanOrdering))
        }
        joinPlanMapSetQueue
      })
      ...
      while (foundPlans.size < items.length) {
        // Build plans for the next level.
        foundPlans += mySearchLevel(foundPlans.toSeq, conf, conditions, topOutputSet, filters,
          foundPlans.size == items.length - 1)
      }
      ...
      // The last level must have one and only one plan, because all items are joinable.
      assert(foundPlans.size == items.length && foundPlans.last.size == 1)
      // logWarning(s"lastLevelPlanNumber is ${foundPlans.last.head._2.size}")
      if (conf.lastLevelSizeEnabled && conf.lastLevelSize < foundPlans.last.head._2.size) {
        conf.setConf(LAST_LEVEL_SIZE, foundPlans.last.head._2.size)
      }
      val finalLevelPlanList = foundPlans.last.head._2.clone.dequeueAll
      // logWarning(s"revisedFinal")
      // for (finalLevelPlan <- finalLevelPlanList) {
      //   logWarning(s"planCost ${finalLevelPlan.planCost}")
      // }
      if (conf.lastLevelIndexEnabled) {
        val lastLevelPlan = foundPlans.last.head._2.toList
        lastLevelPlan(conf.lastLevelIndex.min(lastLevelPlan.size - 1)).plan match {
          case p @ Project(projectList, j: Join) if projectList != output =>
            assert(topOutputSet == p.outputSet)
            // Keep the same order of final output attributes.
            p.copy(projectList = output)
        ...
        // logWarning(s"foundPlanCost ${foundPlans.last.head._2.min(PlanOrdering).planCost}")
        foundPlans.last.head._2.min(PlanOrdering).plan match {
          case p @ Project(projectList, j: Join) if projectList != output =>
            assert(topOutputSet == p.outputSet)
            // Keep the same order of final output attributes.
            p.copy(projectList = output)
...
//                if (existingPlan.isEmpty == false) {
//                  logWarning(s"newJoinPlanItemIds ${newJoinPlan.itemIds}" +
//                    s" ${newJoinPlan.planCost}")
//                }
  ...
  /** Find all possible plans at the next level, based on existing levels. */
  private def mySearchLevel(
                           existingLevels: Seq[JoinPlanMapSetQueue],
                           conf: SQLConf,
                           conditions: ExpressionSet,
                           topOutput: AttributeSet,
                           filters: Option[JoinGraphInfo],
                           isLastLevel: Boolean): JoinPlanMapSetQueue = {
    val nextLevel = new JoinPlanMapSetQueue
    var k = 0
    val lev = existingLevels.length - 1
    // Build plans for the next level from plans at level k (one side of the join) and level
    // lev - k (the other side of the join).
    ...
    while (k <= lev - k) {
      val oneSideCandidates = existingLevels(k).values.toSeq
      for (i <- oneSideCandidates.indices) {
        val oneSidePlanDict = oneSideCandidates(i)
        val otherSideCandidates = if (k == lev - k) {
          // Both sides of a join are at the same level, no need to repeat for previous ones.
          oneSideCandidates.drop(i)
        } else {
        ...
        otherSideCandidates.foreach { otherSidePlanDict =>
          oneSidePlanDict.foreach { oneSidePlan =>
            otherSidePlanDict.foreach { otherSidePlan =>
              buildJoin(oneSidePlan, otherSidePlan, conf, conditions, topOutput, filters) match {
                case Some(newJoinPlan) =>
                  // Check if it's the first plan for the item set, or it's a better plan than
                  // the existing one due to lower cost.
                  val existingPlanDict = nextLevel.get(newJoinPlan.itemIds)
                  if (isLastLevel && conf.lastLevelEpsilonEnabled) {
                    // logWarning(s"setting card ${(1.0 + conf.lastLevelEpsilon) *
                    //   conf.sparkCostCard.toDouble}" +
                    //   s" size ${(1.0 + conf.lastLevelEpsilon) * conf.sparkCostSize.toDouble}")
                    if (newJoinPlan.planCost.card.toDouble <=
                      (1.0 + conf.lastLevelEpsilon) * conf.sparkCostCard.toDouble &&
                      newJoinPlan.planCost.size.toDouble <=
                        (1.0 + conf.lastLevelEpsilon) * conf.sparkCostSize.toDouble) {
                      if (existingPlanDict.isEmpty) {
                        val planPriorityQueue = new mutable.PriorityQueue[JoinPlan]()(PlanOrdering)
                        planPriorityQueue += newJoinPlan
                        nextLevel.update(newJoinPlan.itemIds, planPriorityQueue)
                      }
                      else {
                        val planPriorityQueue = existingPlanDict.get
                        planPriorityQueue += newJoinPlan
                        nextLevel.update(newJoinPlan.itemIds, planPriorityQueue)
                      }
                    }
                  }
                  else if (existingPlanDict.isEmpty) {
                    val planPriorityQueue = new mutable.PriorityQueue[JoinPlan]()(PlanOrdering)
                    planPriorityQueue += newJoinPlan
                    nextLevel.update(newJoinPlan.itemIds, planPriorityQueue)
                  }
                  else if (existingPlanDict.get.size < conf.planGroupSize ||
                    (isLastLevel && conf.maxLastLevelEnabled &&
                      existingPlanDict.get.size < conf.maxLastLevel)) {
                    val planPriorityQueue = existingPlanDict.get
                    planPriorityQueue += newJoinPlan
                    nextLevel.update(newJoinPlan.itemIds, planPriorityQueue)
                  }
                    ...
                    // if (isLastLevel) {
                    //   logWarning(s"lastLevelSizeInSearchLevel is ${existingPlanDict.get.size}")
                    // }
                    val planPriorityQueue = existingPlanDict.get
                    if (newJoinPlan.betterThan(planPriorityQueue.head, conf)) {
                      planPriorityQueue.dequeue()
                      planPriorityQueue += newJoinPlan
//                      logWarning(s"newJoinPlanItemIds ${newJoinPlan.itemIds}" +
//                        s" ${newJoinPlan.planCost}")
                      nextLevel.update(newJoinPlan.itemIds, planPriorityQueue)
//                      if (isLastLevel) {
//                        val finalLevelPlanList = planPriorityQueue.clone.dequeueAll
//                        logWarning(s"newPlanCost ${newJoinPlan.planCost}")
//                        for (finalLevelPlan <- finalLevelPlanList) {
//                          logWarning(s"planCostAfter ${finalLevelPlan.planCost}")
//                        }
//                      }
                    }
                  }
...
//    logWarning(s"current level is ${nextLevel}")
    nextLevel
  }
  ...
  type JoinPlanMapSetQueue = mutable.LinkedHashMap[Set[Int], mutable.PriorityQueue[JoinPlan]]
  ...
  case class JoinPlan(
    ...
    def myBetterThan(other: JoinPlan): Boolean = {
      if (other.planCost.card == 0 || other.planCost.size == 0) {
        false
      } else {
        val relativeRows = BigDecimal(this.planCost.card) / BigDecimal(other.planCost.card)
        val relativeSize = BigDecimal(this.planCost.size) / BigDecimal(other.planCost.size)
        Math.pow(relativeRows.doubleValue, 0.7) *
          Math.pow(relativeSize.doubleValue, 1 - 0.7) < 1
      }
    }
  }
}

object PlanOrdering extends Ordering[JoinPlan] {
  def compare(a: JoinPlan, b: JoinPlan): Int = if (a.myBetterThan(b)) -1 else 1
}
