...
import org.apache.spark.sql.catalyst.plans.logical.{ColumnStat, Histogram, Join, LeafNode, LogicalPlan, Statistics}
...
case class JoinEstimation(join: Join) extends Logging {
  ...
  def isRevised(p: LogicalPlan): Boolean = p match {
    case p: LeafNode => false
    case j: Join => if ((conf.varyCardNodeEnabled &&
      ",".r.replaceAllIn("#[0-9]+".r.replaceAllIn(j.treeString.trim(), ""), "_")
      == conf.varyNodeString)) {
        true
      }
      else {
    ...
    case _: LogicalPlan =>
      val isRevisedSeq = p.children.map(x => isRevised(x))
      isRevisedSeq.contains(true)
  }
  def conf: SQLConf = SQLConf.get
  private val leftStats: Statistics = {
    if ((conf.varyCardEnabled && conf.varyCardSingleEnabled && conf.varyCardLevel
      == countItemSize(join.left)) ||
      (conf.varyCardNodeEnabled && isRevised(join.left))) {
      val rowCount = if (join.left.stats.rowCount.isEmpty) {
        None
      } else {
        val tmpRowCount: Double = join.left.stats.rowCount.get.toDouble / conf.varyCardSwing
        Some(BigDecimal(tmpRowCount).setScale(0, BigDecimal.RoundingMode.HALF_UP)
          .toBigInt.max(1))
      }
      val tmpSizeInBytes: Double = join.left.stats.sizeInBytes.toDouble / conf.varyCardSwing
      val sizeInBytes = BigDecimal(tmpSizeInBytes).setScale(0, BigDecimal.
        RoundingMode.HALF_UP).toBigInt.max(1)
      Statistics(sizeInBytes = sizeInBytes, rowCount = rowCount, attributeStats
        = join.left.stats.attributeStats, isRuntime = join.right.stats.isRuntime)
    }
    else {
      join.left.stats
    }
  }
  private val rightStats: Statistics = {
    if ((conf.varyCardEnabled && conf.varyCardSingleEnabled && conf.varyCardLevel
      == countItemSize(join.right)) ||
      (conf.varyCardNodeEnabled && isRevised(join.right))) {
      val rowCount = if (join.right.stats.rowCount.isEmpty) {
        None
      } else {
        val tmpRowCount: Double = join.right.stats.rowCount.get.toDouble / conf.varyCardSwing
        Some(BigDecimal(tmpRowCount).setScale(0, BigDecimal.RoundingMode.HALF_UP)
        .toBigInt.max(1))
      }
      val tmpSizeInBytes: Double = join.right.stats.sizeInBytes.toDouble / conf.varyCardSwing
      val sizeInBytes = BigDecimal(tmpSizeInBytes).setScale(0, BigDecimal.
        RoundingMode.HALF_UP).toBigInt.max(1)
      // println(s"revisedAttributeStats ${myAttributeStats}")
      Statistics(sizeInBytes = sizeInBytes, rowCount = rowCount, attributeStats
        = join.right.stats.attributeStats, isRuntime = join.right.stats.isRuntime)
    }
    else {
      join.right.stats
    }
  ...
  def estimate: Option[Statistics] = {
    join.joinType match {
      case Inner | Cross | LeftOuter | RightOuter | FullOuter =>
        logDebug(s"enter join estimate: ${join.joinType}")
  ...
  private def estimateInnerOuterJoin(): Option[Statistics] = join match {
    ...
    case ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, _, _, _, _, _) =>
      ...
      // logWarning(s"caseEquiJoin leftRowCount ${leftStats.rowCount.get} rightRowCount " +
        // s"${rightStats.rowCount.get} numInnerJoinedRows ${numInnerJoinedRows}")
