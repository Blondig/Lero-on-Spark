...
// scalastyle:on
  ...
  def simpleString(maxFields: Int): String = {
    // println(s"enter simpleString $nodeName ${argString(maxFields)}")
  ...
  def generateTreeString(
    ...
    // println(s"enter generateTreeString ${str}")
    append(prefix)
    append(str)
    append("\n")
  ...
  def jsonWithStats: String = compact(render(jsonValueWithStats))

  private def jsonValueWithStats: JValue = {
    val jsonValues = scala.collection.mutable.ArrayBuffer.empty[JValue]

    def collectJsonValue(tn: BaseType): Unit = {
      val jsonFields: List[JField] = ("class" -> JString(tn.getClass.getName)) ::
        ("num-children" -> JInt(tn.children.length)) ::
        tn.jsonFieldsWithStatsAppend() ::: tn.jsonFields
      jsonValues += JObject(jsonFields)
      tn.children.foreach(collectJsonValue)
    }
  ...
  // def jsonFieldsWithStats(tn: BaseType): List[JField] = {
  //   ("class" -> JString(tn.getClass.getName)) ::
  //     ("num-children" -> JInt(tn.children.length)) :: tn.jsonFields
  // }

  protected def jsonFieldsWithStatsAppend(): List[JField] = {
      Nil
  }
