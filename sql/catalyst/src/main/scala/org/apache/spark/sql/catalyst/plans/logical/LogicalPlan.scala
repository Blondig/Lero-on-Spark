...
abstract class LogicalPlan
  ...
  override protected def jsonFieldsWithStatsAppend(): List[JField] = statsCache match {
    case Some(s) => if (s.rowCount.isDefined) {
      ("sizeInBytes" -> JInt(s.sizeInBytes)) ::
        ("rowCount" -> JInt(s.rowCount.get)) :: Nil
    } else {
      ("sizeInBytes" -> JInt(s.sizeInBytes)) :: Nil
    }
    case None => Nil
  }
  /**
