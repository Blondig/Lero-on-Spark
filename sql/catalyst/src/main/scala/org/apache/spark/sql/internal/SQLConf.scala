...
object SQLConf {
 ...
 val VARY_CARD_ENABLED =
    buildConf("spark.sql.cbo.varyCard.enabled")
      .doc("Enables vary card in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val VARY_CARD_SINGLE_ENABLED =
    buildConf("spark.sql.cbo.varyCardSingle.enabled")
      .doc("Enables vary card single in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val VARY_CARD_LEVEL =
  buildConf("spark.sql.cbo.varyCard.level")
    .doc("Set vary card level in CBO.")
    .version("3.3.0")
    .intConf
    .createWithDefault(0)

  val VARY_CARD_SWING =
  buildConf("spark.sql.cbo.varyCard.swing")
    .doc("Set vary card swing in CBO.")
    .version("3.3.0")
    .doubleConf
    .createWithDefault(1)

  val ITEM_SIZE_CURRENT_ENABLED =
  buildConf("spark.sql.cbo.itemSizeCurrent.enabled")
      .doc("Enables item size in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val ITEM_SIZE_CURRENT =
  buildConf("spark.sql.cbo.itemSizeCurrent")
      .doc("Current item size in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(0)

  val JOIN_PLAN_SEQ_ENABLED =
  buildConf("spark.sql.cbo.joinPlanSeq.enabled")
      .doc("Enables join plan seq in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val JOIN_PLAN_SEQ =
   buildConf("spark.sql.cbo.joinPlanSeq")
      .version("3.3.0")
      .stringConf
      .toSequence
      .createWithDefault(Nil)

  val VARY_CARD_NODE_ENABLED =
    buildConf("spark.sql.cbo.varyCardNode.enabled")
      .doc("Enables vary card node in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val VARY_NODE_STRING =
    buildConf("spark.sql.cbo.")
      .doc("Vary node string in CBO.")
      .version("3.3.0")
      .stringConf
      .createWithDefault("")

   val PLAN_GROUP_ENABLED =
    buildConf("spark.sql.cbo.planGroup.enabled")
      .doc("Enables plan group in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val PLAN_GROUP_SIZE =
    buildConf("spark.sql.cbo.planGroupSize")
      .doc("Plan group size in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(0)

  val LAST_LEVEL_SIZE_ENABLED =
    buildConf("spark.sql.cbo.lastLevelSize.enabled")
      .doc("Enables last level size in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val LAST_LEVEL_SIZE =
    buildConf("spark.sql.cbo.lastLevelSize")
      .doc("Last level size in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(0)

  val LAST_LEVEL_INDEX_ENABLED =
    buildConf("spark.sql.cbo.lastLevelIndex.enabled")
      .doc("Enables last level index in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val LAST_LEVEL_INDEX =
    buildConf("spark.sql.cbo.lastLevelIndex")
      .doc("Set last level index in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(0)

  val MAX_LAST_LEVEL_ENABLED =
    buildConf("spark.sql.cbo.maxLastLevel.enabled")
      .doc("Enables max last level in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val MAX_LAST_LEVEL =
    buildConf("spark.sql.cbo.maxLastLevel")
      .doc("Set max last level in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(10000)

  val SPARK_COST_RECORD_ENABLED =
    buildConf("spark.sql.cbo.recordSparkCost.enabled")
      .doc("Enables spark cost recording in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val SPARK_COST_CARD =
    buildConf("spark.sql.cbo.sparkCostCard")
      .doc("Spark cost card in CBO.")
      .version("3.3.0")
      .longConf
      .createWithDefault(0)

  val SPARK_COST_SIZE =
    buildConf("spark.sql.cbo.sparkCostSize")
      .doc("Spark cost size in CBO.")
      .version("3.3.0")
      .longConf
      .createWithDefault(0)

  val LAST_LEVEL_EPSILON_ENABLED =
    buildConf("spark.sql.cbo.lastLevelEpsilon.enabled")
      .doc("Enables last level epsilon in CBO.")
      .version("3.3.0")
      .booleanConf
      .createWithDefault(false)

  val LAST_LEVEL_EPSILON =
    buildConf("spark.sql.cbo.lastLevelEpsilon")
      .doc("Last level epsilon in CBO.")
      .version("3.3.0")
      .doubleConf
      .createWithDefault(0.0)

  val TABLE_NUM_LOW =
    buildConf("spark.sql.cbo.tableNumLow")
      .doc("Set table number low bound in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(0)

  val TABLE_NUM_UP =
    buildConf("spark.sql.cbo.tableNumUp")
      .doc("Set table number up bound in CBO.")
      .version("3.3.0")
      .intConf
      .createWithDefault(10000)
...
class SQLConf extends Serializable with Logging {
  ...
  def varyCardEnabled: Boolean = getConf(SQLConf.VARY_CARD_ENABLED)

  def varyCardSingleEnabled: Boolean = getConf(SQLConf.VARY_CARD_SINGLE_ENABLED)

  def varyCardLevel: Int = getConf(SQLConf.VARY_CARD_LEVEL)

  def varyCardSwing: Double = getConf(SQLConf.VARY_CARD_SWING)

  def itemSizeCurrentEnabled: Boolean = getConf(SQLConf.ITEM_SIZE_CURRENT_ENABLED)

  def itemSizeCurrent: Int = getConf(SQLConf.ITEM_SIZE_CURRENT)

  def joinPlanSeqEnabled: Boolean = getConf(SQLConf.JOIN_PLAN_SEQ_ENABLED)

  def joinPlanSeq: Seq[String] = getConf(SQLConf.JOIN_PLAN_SEQ)

  def varyCardNodeEnabled: Boolean = getConf(SQLConf.VARY_CARD_NODE_ENABLED)

  def varyNodeString: String = getConf(SQLConf.VARY_NODE_STRING)

  def planGroupEnabled: Boolean = getConf(SQLConf.PLAN_GROUP_ENABLED)

  def planGroupSize: Int = getConf(SQLConf.PLAN_GROUP_SIZE)

  def lastLevelSizeEnabled: Boolean = getConf(SQLConf.LAST_LEVEL_SIZE_ENABLED)

  def lastLevelSize: Int = getConf(SQLConf.LAST_LEVEL_SIZE)

  def lastLevelIndexEnabled: Boolean = getConf(SQLConf.LAST_LEVEL_INDEX_ENABLED)

  def lastLevelIndex: Int = getConf(SQLConf.LAST_LEVEL_INDEX)

  def maxLastLevelEnabled: Boolean = getConf(SQLConf.LAST_LEVEL_EPSILON_ENABLED)

  def maxLastLevel: Int = getConf(SQLConf.MAX_LAST_LEVEL)

  def sparkCostRecordEnabled: Boolean = getConf(SQLConf.SPARK_COST_RECORD_ENABLED)

  def sparkCostCard: Long = getConf(SQLConf.SPARK_COST_CARD)

  def sparkCostSize: Long = getConf(SQLConf.SPARK_COST_SIZE)

  def lastLevelEpsilonEnabled: Boolean = getConf(SQLConf.LAST_LEVEL_EPSILON_ENABLED)

  def lastLevelEpsilon: Double = getConf(SQLConf.LAST_LEVEL_EPSILON)

  def tableNumLow: Int = getConf(SQLConf.TABLE_NUM_LOW)

  def tableNumUp: Int = getConf(SQLConf.TABLE_NUM_UP)
