...
abstract class SparkPlan extends QueryPlan[SparkPlan] with Logging with Serializable {
  ...
  var sparkPlanRowCount: Option[BigInt] = Some(0)

  var sparkPlanSizeInBytes: BigInt = 0

  override protected def jsonFieldsWithStatsAppend(): List[JField] = {
    if (sparkPlanRowCount.isDefined) {
      ("sizeInBytes" -> JInt(sparkPlanSizeInBytes)) ::
        ("rowCount" -> JInt(sparkPlanRowCount.get)) :: Nil
    }
    else {
      ("sizeInBytes" -> JInt(sparkPlanSizeInBytes)) :: Nil
    }
  }
  ...
  private def setLogicalLink(logicalPlan: LogicalPlan, inherited: Boolean = false): Unit = {
    ...
    if (conf.varyCardNodeEnabled && isRevised(logicalPlan)) {
      this.sparkPlanRowCount = if (logicalPlan.stats.rowCount.isEmpty) {
        None
      }
      else {
        val tmpRowCount: Double = logicalPlan.stats.rowCount.get.toDouble / conf.varyCardSwing
        Some(BigDecimal(tmpRowCount).setScale(0, BigDecimal.RoundingMode.HALF_UP)
          .toBigInt.max(1))
      }
      val tmpSizeInBytes: Double = logicalPlan.stats.sizeInBytes.toDouble / conf.varyCardSwing
      this.sparkPlanSizeInBytes = BigDecimal(tmpSizeInBytes).setScale(0, BigDecimal.
        RoundingMode.HALF_UP).toBigInt.max(1)
    }
    else {
      this.sparkPlanRowCount = logicalPlan.stats.rowCount
      this.sparkPlanSizeInBytes = logicalPlan.stats.sizeInBytes
    }
