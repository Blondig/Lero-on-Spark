...
class QueryExecution(
  ...
  lazy val optimizedPlan: LogicalPlan = {
    ...
    val tempPlan = executePhase(QueryPlanningTracker.OPTIMIZATION) {
      // clone the plan to avoid sharing the plan instance between different stages like analyzing,
      // optimizing and planning.
      val plan =
    ...
    try {
      ...
      // to the node.
      tempPlan.collectWithSubqueries {
        case plan => plan.stats
      }
    } catch {
      case e: AnalysisException => logWarning(e.toString)
    }
    // println("test optimized plan")
    tempPlan
  }
  ...
  def stringWithStats: String = {
    // println("enter my new stringWithStats xxx")
    val concat = new PlanStringConcat()
  ...
  private def stringWithStats(maxFields: Int, append: String => Unit): Unit = {
    ...
    logWarning(s"system print plans")
    logWarning(s"enter stringWithStats")
    append("== Optimized Logical Plan ==\n")
    QueryPlan.append(optimizedPlan, append, verbose = true, addSuffix = true, maxFields)
    // println("show optimized JSON")
    // println(optimizedPlan.toJSON)
    // println("show optimized string")
