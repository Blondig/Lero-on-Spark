...
import org.apache.spark.sql.internal.SQLConf.JOIN_PLAN_SEQ_ENABLED
import org.apache.spark.sql.internal.SQLConf.JOIN_PLAN_SEQ
import org.apache.spark.sql.internal.SQLConf.VARY_CARD_NODE_ENABLED
import org.apache.spark.sql.internal.SQLConf.VARY_NODE_STRING
...
for(query <- queryList) {
    ...
    sqlConf.setConf(VARY_CARD_NODE_ENABLED, false)
    sqlConf.setConf(JOIN_PLAN_SEQ_ENABLED, true)
    sqlConf.setConf(JOIN_PLAN_SEQ, Seq())
    println(s"joinPlanSeqSizeBefore ${sqlConf.joinPlanSeq.size}")
    ...
    val joinPlanSeq = sqlConf.joinPlanSeq
    sqlConf.setConf(JOIN_PLAN_SEQ_ENABLED, false)
    ...
    val newString = regexJsonId.replaceAllIn(planStringOriginal, "")
    val planString = regexJsonJvmId.replaceAllIn(newString, "")
    ...
    println(s"joinPlanSeqSizeAfter ${sqlConf.joinPlanSeq.size}")
    
    println(s"Current Set is ${planHashMap.keySet}")
    println(s"query${queryIndex} original elapsed time ${elapsed}")
    
    sqlConf.setConf(VARY_CARD_NODE_ENABLED, true)
     // generate candidate plans using swing factors
    val swingList = List(0.01, 0.1, 10.0, 100.0) 
    for ((joinPlan, level) <- joinPlanSeq.view.zipWithIndex) {
        for (swing <- swingList) {
            try {
                sqlConf.setConf(VARY_NODE_STRING, joinPlan)
